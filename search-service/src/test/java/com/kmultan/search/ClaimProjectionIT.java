...
class ClaimProjectionIT {
    ...
    private void publish(UUID claimId, long sequence, String json) throws Exception {
        ...
        producerRecord
                .headers()
                .add(new RecordHeader("sequence", Long.toString(sequence).getBytes(StandardCharsets.UTF_8)));
    ...
    void projectsEventsAndSupportsFuzzySearch() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(20)).untilAsserted(() -> assertThat(
                        claimSearchService.search("bumper", null, 0, 10).items())
                .anyMatch(document -> document.claimId().equals(id)));

        // fuzzy plate match (one typo)
        assertThat(claimSearchService.search("WA12354", null, 0, 10).items())
                .anyMatch(document -> document.claimId().equals(id));
        ...
        publish(
                id,
                11,
                event(id, "ASSESSMENT_STARTED", "UNDER_ASSESSMENT", "WA12345", "Rear bumper dented in a parking lot"));
        await().atMost(Duration.ofSeconds(20)).untilAsserted(() -> assertThat(claimSearchService
                        .search(null, "UNDER_ASSESSMENT", 0, 10)
                        .items())
                .anyMatch(document -> document.claimId().equals(id)));

        publish(id, 10, event(id, "CLAIM_SUBMITTED", "SUBMITTED", "WA12345", "Rear bumper dented in a parking lot"));
        Thread.sleep(1500);
        assertThat(claimSearchService.search(null, "UNDER_ASSESSMENT", 0, 10).items())
                .anyMatch(document -> document.claimId().equals(id));

        // the event log keeps every distinct fact (3 event ids were published); the projection kept only the newest
        // state
        elasticsearchClient.indices().refresh(request -> request.index("claim-events"));
        long logged = elasticsearchClient
                .count(request -> request.index("claim-events")
                        .query(query -> query.term(term -> term.field("claimId").value(id.toString()))))
                .count();
