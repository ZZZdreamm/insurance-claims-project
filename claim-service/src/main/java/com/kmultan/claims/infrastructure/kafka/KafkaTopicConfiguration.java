...
public class KafkaTopicConfiguration {
    ...
    @Bean
    public NewTopic claimsEventsTopic(@Value("${claims.topics.claims}") String name) {
        return topic(name);
    }

    @Bean
    public NewTopic assessmentEventsTopic(@Value("${claims.topics.assessment}") String name) {
        return topic(name);
    }

    @Bean
    public NewTopic payoutEventsTopic(@Value("${claims.topics.payout}") String name) {
        return topic(name);
    }

    @Bean
    public NewTopic assessmentEventsDeadLetterTopic(@Value("${claims.topics.assessment}") String name) {
        return topic(name + KafkaDeadLetterConfiguration.DEAD_LETTER_SUFFIX);
    }

    @Bean
    public NewTopic payoutEventsDeadLetterTopic(@Value("${claims.topics.payout}") String name) {
        return topic(name + KafkaDeadLetterConfiguration.DEAD_LETTER_SUFFIX);
    }
}
