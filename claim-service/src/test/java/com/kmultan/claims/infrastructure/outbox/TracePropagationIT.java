...
class TracePropagationIT extends AbstractIntegrationTest {
    ...
    void kafkaRecordCarriesTheOriginatingTraceId() {
        ...
        try (Tracer.SpanInScope ignored = tracer.withSpan(submitSpan)) {
            claim = claimService.submit("POL-TR", "TR 1", LocalDate.now(), "Trace propagation integration test", null, List.of());
        } finally {
            submitSpan.end();
        }
        String traceId = submitSpan.context().traceId();

        OutboxEvent submittedRow = outboxEvents.findByAggregateIdOrderById(claim.getId()).get(0);
        ...
        try (KafkaTestConsumer claimsTopicConsumer = new KafkaTestConsumer(KAFKA.getBootstrapServers(), claimsTopic)) {
            await().atMost(Duration.ofSeconds(15)).untilAsserted(() ->
                    assertThat(claimsTopicConsumer.pollForKey(claim.getId().toString())).isNotEmpty());
            ConsumerRecord<String, String> submitted = claimsTopicConsumer.pollForKey(claim.getId().toString()).get(0);
